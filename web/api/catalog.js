...
async function eiaSearch(query,key){
  if(!key||query.length<2)return[]; const q=normalize(query); const ranked=Object.entries(branchHints).map(([id,name])=>({id,rank:score(q,{id,name,source:"EIA"})})).sort((a,b)=>b.rank-a.rank);const branches=ranked.filter((row)=>row.rank>.28).slice(0,2).map((row)=>row.id);if(!branches.length)branches.push("petroleum","total-energy");
  const leaves=(await Promise.all(branches.map((branch)=>discoverLeaves(branch,key)))).flat(); const top=leaves.map((leaf)=>({...leaf,rank:score(q,{id:leaf.route,name:`${leaf.metadata.name||""} ${leaf.metadata.description||""}`,source:"EIA"})})).sort((a,b)=>b.rank-a.rank).slice(0,8);
...
export default async function handler(request,response){
  ...
  const rows=unique([...builtIn,...fred,...eia]).map(({providerId:_providerId,_score,...item})=>item);response.setHeader("Cache-Control","public, s-maxage=1800, stale-while-revalidate=21600");response.status(200).json({items:rows,coverage:{FRED:Boolean(process.env.FRED_API_KEY),EIA:true},warnings});
}
