...
def market_refresh_strategy() -> dict[str, bool | int]:
    ...
    Every interactive run first reuses a cache only when it is recent enough
    for the requested end date. Missing or stale series still fall through to
    their online sources. This keeps both local and hosted workspaces responsive
    when a public provider is temporarily empty or rate-limited.
    """
...
def render_main_header(active_workspace: str = "decision") -> str:
    """Render the only product-mode switch and the responsive utility dock."""
    st.markdown(
        f'<a class="skip-link" href="#main-content">{ui_text("Skip to main content", "跳到主要内容")}</a>',
        unsafe_allow_html=True,
    )
    brand_col, mode_col, tools_col = st.columns([0.31, 0.43, 0.26], gap="small")
    with brand_col:
        st.markdown(
    ...
    with mode_col:
        st.markdown('<span class="mode-dock-anchor" aria-hidden="true"></span>', unsafe_allow_html=True)
        selected_mode = st.segmented_control(
            ui_text("Product mode", "产品模式"),
            ["decision", "professional"],
            format_func=lambda value: ui_text("Decision", "决策模式")
            if value == "decision"
            else ui_text("Professional", "专业模式"),
            label_visibility="collapsed",
            key="primary_workspace_mode",
        )
    with tools_col:
        st.markdown('<span class="tool-dock-anchor" aria-hidden="true"></span>', unsafe_allow_html=True)
        settings_col, language_col = st.columns([1.06, 0.94], gap=None)
        with settings_col:
            render_top_tool_menu()
        with language_col:
            render_language_switcher()

    active_workspace = selected_mode or active_workspace
    if active_workspace == "decision":
        st.markdown(
            f"""
            <section class="product-hero" id="main-content">
              <div class="hero-data-field" aria-hidden="true">
                <svg viewBox="0 0 1200 600" preserveAspectRatio="none">
                  <path class="flow flow-a" d="M-60 452 C175 336 265 540 470 390 S795 194 1260 286" />
                  <path class="flow flow-b" d="M-40 382 C210 246 332 414 508 322 S830 112 1240 218" />
                  <path class="flow flow-c" d="M-20 515 C250 420 390 580 610 442 S962 284 1260 356" />
                  <circle class="node node-a" cx="470" cy="390" r="8" />
                  <circle class="node node-b" cx="795" cy="194" r="6" />
                  <circle class="node node-c" cx="1030" cy="247" r="10" />
                </svg>
              </div>
              <div class="hero-content">
                <p class="hero-kicker">{ui_text("OIL SIGNALS, TRANSLATED", "把油价信号变成行动依据")}</p>
                <h1>{ui_text("Read the move. Price the risk. Plan the response.", "看懂波动，算清风险，提前应对")}</h1>
                <p>{ui_text("A continuous oil-market view from linked drivers and multiscale forecasts to volatility risk, procurement cost and bounded research actions.", "把近期关联因素、多尺度预测和高波动风险连成一条线，再换算成采购成本、套保选择与投资研究边界。")}</p>
                <div class="hero-actions">
                  <a class="hero-action-primary" href="#decision-output">{ui_text("Open today's brief", "查看本期判断")}<span aria-hidden="true">↓</span></a>
                </div>
                <div class="hero-sequence" aria-label="{ui_text('Decision workflow', '决策路径')}">
                  <span>{ui_text("Drivers", "关联因素")}</span><i></i>
                  <span>{ui_text("Forecast", "价格预测")}</span><i></i>
                  <span>{ui_text("Risk", "风险预警")}</span><i></i>
                  <span>{ui_text("Cost", "成本测算")}</span><i></i>
                  <span>{ui_text("Action", "行动建议")}</span>
                </div>
              </div>
            </section>
            """,
        ...
        st.markdown('<div id="main-content" class="professional-content-anchor"></div>', unsafe_allow_html=True)
    ...
    if run_forecast:
        ...
        try:
            ...
            status.write(ui_text(
                f"Loading the latest verified {target} data; online sources are contacted only when needed.",
                f"正在读取最近成功的 {target} 数据；仅在本地数据不足时连接在线源。",
            ))
            ...
            if price_data.empty or target not in price_data or price_data[target].notna().sum() < 180:
                raise ValueError(ui_text(
                    ...
                    f"暂时没有足够的新近 {target} 数据，请检查网络或数据源凭据。",
                ))
    ...
    if result is None:
        st.markdown(
            ...
            <div class="empty-state">
                ...
                <p>{ui_text("Set the target, horizon and sample length, then generate a forecast.", "设置预测对象、预测期限和历史样本长度后，点击“读取最近成功数据并生成预测”。")}</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def sync_primary_workspace_from_query() -> None:
    """Use a deep-linked product mode only before the mode widget is created."""
    if "primary_workspace_mode" in st.session_state:
        return
    try:
    ...
    if workspace not in {"decision", "professional"}:
        return
...
def load_decision_dashboard_module() -> Any:
    """Import the decision dashboard once; Streamlit handles source changes."""
    return importlib.import_module("app.executive_dashboard")


@st.fragment
def render_application_workspace(options: dict[str, Any]) -> None:
    """Render the interactive workspace without rerunning global setup.

    Product-mode, language and tool interactions stay inside this fragment, so
    Streamlit preserves the page shell and animated background while swapping
    the active workspace.
    """
    stored_mode = st.session_state.get("primary_workspace_mode", "decision")
    active_workspace = "professional" if stored_mode == "professional" else "decision"
    st.session_state["primary_workspace_mode"] = active_workspace
    active_workspace = render_main_header(active_workspace)
    try:
        st.query_params["workspace"] = active_workspace
        st.query_params.pop("request", None)
    except Exception:  # pragma: no cover - older Streamlit query proxies.
        pass
    if active_workspace == "decision":
    ...
    st.markdown(
        f"""
        <section class="section-intro professional-intro view-reveal">
          <p class="section-kicker">{ui_text("RESEARCH WORKSPACE", "专业工作台")}</p>
          <h2>{ui_text("Research tools, ready where you left them", "直接进入需要的研究工具")}</h2>
          <p>{ui_text("Switch tools without an intermediate home page. Date windows, IMF counts, validation and every intermediate chart remain available inside the selected workflow.", "无需经过单独首页，直接切换净影响、价格预测、风险预警和连接数据；时间范围、IMF 数量、验证结果与中间图表都保留。")}</p>
        </section>
        """,
        unsafe_allow_html=True,
    )
...
def main() -> None:
    ...
    sync_primary_workspace_from_query()
    render_application_workspace(options)
